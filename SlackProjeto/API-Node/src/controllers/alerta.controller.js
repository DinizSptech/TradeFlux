...
async function enviarJira(req, res) {
  try {
    ...
    if (criticidade !== 3) {
      return res
        .status(200)
        .send("Alerta recebido, mas criticidade != 3. Não enviado ao Jira.");
    }
    ...
    await bdModel.insert_alerta_com_issueKey(
      valor,
      medida,
      data,
      criticidade,
    ...
    res
      .status(201)
      .send(`Alerta crítico criado no Jira com issueKey ${issueKey}`);
  } catch (err) {
